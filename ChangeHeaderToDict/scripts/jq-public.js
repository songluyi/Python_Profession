...
function getLochis() {
    ...
    $("#selecthis").hover(function () {
        ...
        $.ajax({
            type: "get",
            url: "/ajaxsync.aspx?at=qh&val=" + encodeURIComponent(cookie),
            beforeSend: function () {
                $(".BomreList").html("<div class=\"BorWrapa tc\"><img src=\"/template/default/images/public/spinner.gif\" class=\"mt10\" /></div>");
            },
...
function bindClick() {
    $("i.jsclear").click(function () {
        var _this = this;
        $.post("/ajaxsync.aspx?at=clearqh&val=" + encodeURIComponent($(this).attr("v")), function (data) {
            if (data == 1) {
    ...
    $("#jsclearall").click(function () {
        $.post("/ajaxsync.aspx?at=clearqh&val=all", function (data) {
